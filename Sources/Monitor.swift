...
public class QuotaMonitor: ObservableObject {
    ...
    public func updateAccountTokens(id: String, accessToken: String, refreshToken: String, accountId: String? = nil, email: String? = nil) {
        if let idx = config.accounts.firstIndex(where: { $0.id == id }) {
            ...
            Task {
                let (status, updatedConfig) = await APIClient.shared.fetchQuota(for: targetAccount)
                self.statuses[id] = status
                if let updated = updatedConfig {
                    self.config.accounts[idx] = updated
